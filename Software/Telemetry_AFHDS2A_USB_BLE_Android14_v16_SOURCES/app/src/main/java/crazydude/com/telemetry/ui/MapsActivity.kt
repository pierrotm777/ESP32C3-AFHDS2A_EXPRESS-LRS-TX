...
import crazydude.com.telemetry.compat.BleCompat
...
import crazydude.com.telemetry.utils.StorageCompat
...
class MapsActivity : AppCompatActivity(), DataDecoder.Listener, SensorTimeoutManager.Listener, CameraFragmentListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        */
        ...
        if (isInReplayMode()) {
            startReplay(
                File(
                    StorageCompat.telemetryLogDir(this),
                    replayFileString
                )
            )
        } else {
    ...
    private fun replay() {
        ...
        val dir = StorageCompat.telemetryLogDir(this)
        val files = dir.listFiles { file ->
            ((file.extension == "log") || (file.extension == "tlm")) && (file.length() > 0)
        }?.sorted()?.reversed() ?: emptyList()
        ...
        val dialog = AlertDialog.Builder(this)
            .setAdapter(
                ArrayAdapter(
                    ...
                    files.map { file ->
                        if (file.name == lastFileDialogSelection) {
                            val label = "${file.nameWithoutExtension} (${ceil(file.length() / 102.4) / 10} Kb)"
                            SpannableString(label).apply {
                                setSpan(StyleSpan(Typeface.BOLD), 0, label.length, 0)
                            }
                        } else {
                            "${file.nameWithoutExtension} (${ceil(file.length() / 102.4) / 10} Kb)"
                        }
                    }
                )
            ) { _, index ->
                updateWindowFullscreenDecoration()
                lastFileDialogSelectionIndex = index
                lastFileDialogSelection = files[index].name
                startReplay(files[index])
            }
            .create()
        ...
        dialog.setOnShowListener {
            ...
            if (lastFileDialogSelectionIndex != -1) {
                val centerY = alertDialog.listView.height / 2
    ...
    private fun startReplay(file: File?) {
        file?.also {
            ...
            this.logPlayer = LogPlayer(this, this)
    ...
    private fun connectUSB() {
        ...
        if (driver == null) {
            ...
            if (connection != null) {
                ...
                val pendingIntent =
                    PendingIntent.getBroadcast(this, 0, Intent(ACTION_USB_DEVICE), PendingIntent.FLAG_IMMUTABLE)
                ...
                }, IntentFilter(ACTION_USB_DEVICE), ContextCompat.RECEIVER_NOT_EXPORTED)
    ...
    private fun connectBluetooth() {
        if (!BleCompat.ensurePermissions(this)) {
            Toast.makeText(
                this,
                "Bluetooth permission is required. Please try again after allowing it.",
                Toast.LENGTH_LONG
            ).show()
            return
        }
        ...
        val devices = ArrayList<BluetoothDevice>(BleCompat.getBondedDevices(adapter))
    ...
    private fun bleCheck() =
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2 && BleCompat.ensurePermissions(this)
    ...
    private fun startDataService() {
        ...
        // Do not use startForegroundService here: DataService only becomes foreground
        // after an actual telemetry connection is established. Android 8+ requires a
        // service started with startForegroundService() to call startForeground()
        // within five seconds, which caused the application to be killed at startup.
        startService(intent)
            ...
            if (requestCode == REQUEST_LOCATION_PERMISSION) {
                ...
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    when (requestWritePermissionSequence) {
                        ...
                        RequestWritePermissionSequenceType.NONE -> Unit
                    }
    ...
    private fun renameLog(currentFileName: String, newFileName: String) {
        val currentFile = File(StorageCompat.telemetryLogDir(this), currentFileName)
        val newFile = File(StorageCompat.telemetryLogDir(this), newFileName)
        ...
        if (currentFile.renameTo(newFile)) {
            ...
            val csvCurrentFile = File(StorageCompat.telemetryLogDir(this), csvCurrentFileName)
            val csvNewFile = File(StorageCompat.telemetryLogDir(this), csvNewFileName)
    ...
    {
        val currentFile = File(StorageCompat.telemetryLogDir(this), fileName)

        if (currentFile.delete()) {
            Toast.makeText(this, "Log deleted successfully.", Toast.LENGTH_SHORT).show()

            val csvFileName = replaceExtension( fileName, ".csv")
            val currentFileCSV = File(StorageCompat.telemetryLogDir(this), csvFileName)
    ...
    fun requestWritePermission(seq: RequestWritePermissionSequenceType): Boolean {
        // Android 14 build stores logs and exports in app-scoped external storage.
        // No legacy READ/WRITE_EXTERNAL_STORAGE runtime permission is required.
