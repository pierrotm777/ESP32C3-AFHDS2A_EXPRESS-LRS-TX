...
class LogPlayer(private val context: Context, val originalListener: DataDecoder.Listener) : DataDecoder.Listener {
                ...
                ProtocolDetector(object :
                    ...
                    override fun onProtocolDetected(detectedProtocol: Protocol?) {
                        when (detectedProtocol) {
                            ...
                            is Afhds2aProtocol -> {
                                tempProtocol = Afhds2aProtocol(tempDecoder)
                                protocol = Afhds2aProtocol(this@LogPlayer)
                                dataReadyListener?.onProtocolDetected("FlySky AFHDS2A")
                            }
                        }
                    }
                })
    ...
    {
        val dir = StorageCompat.telemetryLogDir(context)
